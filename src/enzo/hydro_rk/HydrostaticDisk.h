...
/  modified3:  Wolfram Schmidt, July 2023
/
...
public:
        ...
        hydrostatic_disk(int N1, int N2, double delta1, double delta2, double scale1, double scale2, double sig, double sound_speed)
        {
                equipart = true;
                pressureGradientType = 2;
                N_r=N1;
                ...
                sigma=sig;
                magn_fract=0.0;
                rho_min=0.0; // WS: don't use density floor for integration
                ...
                for(int i=0;i<N_r;i++)
                {
                        ...
                        // WS: factor of two following Wang+2010, Eq. 22
                        data[i][0].rho = 0.5*sigma*exp(-data[i][0].r/r_sc)/z_sc;
...
*/
        ...
        double halo_vel_sqr(double r, double z)
        {
